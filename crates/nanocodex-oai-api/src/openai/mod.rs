...
impl<F> OpenAiBuilder<F> {
    ...
    ///
    /// For example, an API-key HTTPS router may expose the supported Sol model as
    ...
    /// This does not admit arbitrary models or change snapshot model identity.
    /// [`Self::build`] rejects empty or malformed prefixes and prefixes used
    /// with ChatGPT authentication or the WebSocket transport.
    #[must_use]
    pub fn model_id_prefix(mut self, prefix: impl Into<String>) -> Self {
        let prefix = prefix.into();
        self.config.model_id_prefix = Some(Arc::from(prefix.trim().trim_end_matches('/')));
        self
    }
...
mod tests {
    ...
    fn model_id_prefix_is_normalized_and_scoped_to_api_key_https() {
        ...
        for prefix in [" ", "/openai", "openai//routing", "open ai"] {
