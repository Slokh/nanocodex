    ...
    fn resolve(&self) -> Result<String> {
        match self {
            ...
            Self::Guest(version) => version
                .get()
                .cloned()
                .ok_or_else(|| eyre!("stock Codex did not report its version inside the guest")),
        }
    }
}
...
impl DiffProgress {
    async fn start(path: PathBuf, started: Instant) -> Result<(Self, DiffProgressRecorder)> {
    ...
    ) -> Result<(Self, DiffProgressRecorder)> {
    ...
    fn observe_nanocodex(&self, event: &nanocodex::agent::events::AgentEvent) {
...
impl DiffProgressRecorder {
    async fn finish(self, progress: DiffProgress) -> Result<()> {
        drop(progress);
        self.task
            .await
...
impl Diff {
    pub(crate) async fn run(self) -> Result<()> {
        ...
        let output_parent = prepare_output_parent(&self.output)?;
        ...
        progress.emit(
            "runner",
            "comparison.started",
            format!("{} · {} / {}", task.name(), nanocodex::oai::MODEL, thinking),
        );
        ...
        let profile_validation_error = validate_matched_code_mode_only_profile(
            &api_comparison,
            nanocodex::oai::MODEL,
            thinking.as_str(),
        );
        ...
        let report = ComparisonReport {
            ...
            api_comparison,
            nanocodex_build: ExecutableIdentity {
                path: current_executable.clone(),
                version: env!("NANOCODEX_SEMVER_VERSION").to_owned(),
                git_sha: Some(env!("VERGEN_GIT_SHA").to_owned()),
                built_at: Some(env!("VERGEN_BUILD_TIMESTAMP").to_owned()),
                sha256: file_sha256(&current_executable)?,
            },
            codex_build: ExecutableIdentity {
                path: codex_path,
...
impl ComparisonReport {
    fn has_operational_error(&self) -> bool {
    ...
    fn write_human(&self) {
        println!("{}", self.classification.as_str());
        ...
        if let Some(path) = &self.artifacts.api_comparison {
            println!("API comparison: {}", path.display());
        }
        if let Some(error) = &self.artifacts.api_comparison_error {
            println!("API comparison error: {error}");
        }
        if let Some(error) = &self.artifacts.profile_validation_error {
            println!("matched-profile error: {error}");
        }
        println!("comparison: {}", self.artifacts.comparison.display());
    }
}
...
impl ArmReport {
    fn from_outcome(
        ...
        event_error: Option<String>,
        trajectory: Result<TrajectoryArtifact, String>,
...
async fn record_events(
    mut stream: nanocodex_eval::EvalEventStream,
    path: &Path,
    arm_name: &'static str,
    progress: DiffProgress,
...
fn retain_trajectory(
    ...
    let trajectory = match projection {
        ...
        TrajectoryProjection::Codex { version } => {
            let agent = outcome_agent(outcome)
                .ok_or_else(|| eyre!("stock Codex attempt retained no terminal agent result"))?;
...
fn retain_arm_api_exchanges(
    ...
    codex_arm: bool,
    required: bool,
) -> Result<Option<ApiCaptureArtifact>> {
...
fn project_nanocodex_api_exchanges(event_log: &Path, output: &Path) -> Result<()> {
    let parent = output
        .parent()
        .ok_or_else(|| eyre!("API exchange path has no parent: {}", output.display()))?;
    fs::create_dir_all(parent)?;
    let input =
...
fn reanalyze_comparison(path: &Path, json: bool) -> Result<()> {
    let requested = path
        .canonicalize()
    ...
    let comparison_object = comparison
        .as_object_mut()
        .ok_or_else(|| eyre!("retained comparison root is not an object"))?;
    ...
    let nanocodex = comparison_object
        ...
        .ok_or_else(|| eyre!("retained comparison has no Nanocodex arm object"))?;
    nanocodex.insert(
    ...
    let codex = comparison_object
        .get_mut("codex")
        .and_then(serde_json::Value::as_object_mut)
        .ok_or_else(|| eyre!("retained comparison has no Codex arm object"))?;
    codex.insert(
        "trajectory_summary".to_owned(),
    ...
    let artifacts = comparison_object
        .get_mut("artifacts")
        .and_then(serde_json::Value::as_object_mut)
        .ok_or_else(|| eyre!("retained comparison has no artifacts object"))?;
    artifacts.insert(
    ...
    if json {
        let report = if api_summary.is_some() {
        ...
        write_shell_polling_summary("nanocodex", &nanocodex_trajectory_summary.shell_polling);
        write_shell_polling_summary("codex", &codex_trajectory_summary.shell_polling);
        if let Some(summary) = &api_summary {
            ...
            write_event_loop_arm_summary("nanocodex", summary.event_loop.nanocodex.as_ref());
            write_event_loop_arm_summary("codex", summary.event_loop.codex.as_ref());
            if let Some(error) = &profile_validation_error {
                println!("matched-profile error: {error}");
            }
            println!("API comparison: {}", api_comparison_path.display());
        }
...
fn required_retained_artifact_path(
    ...
    retained_artifact_path(comparison, directory, pointer, arm, artifact)?
        .ok_or_else(|| eyre!("{arm} comparison arm has no retained {artifact} path"))
}
...
fn write_event_loop_arm_summary(name: &str, summary: Option<&ApiEventLoopArmSummary>) {
    let Some(summary) = summary else {
        println!("{name} event loop: unavailable");
        return;
    };
    println!(
...
fn read_api_request_payloads(path: &Path) -> Result<Vec<ApiRequestPayload>> {
...
fn write_json_atomic(path: &Path, value: &impl Serialize) -> Result<()> {
    let parent = path
        .parent()
        .ok_or_else(|| eyre!("comparison path has no parent: {}", path.display()))?;
...
fn write_arm(name: &str, arm: &ArmReport) {
    ...
    if let Some(error) = &arm.operational_error {
        println!("{name} runner error: {error}");
    }
    if let Some(error) = &arm.event_error {
        println!("{name} event error: {error}");
    }
    if let Some(error) = &arm.trajectory_error {
        println!("{name} trajectory error: {error}");
    }
}
...
mod tests {
    ...
    use nanocodex_eval::{AgentStatus, EvalAttemptOutcome, EvalStatus};
    ...
    use super::{
        ...
        inspect_api_exchanges, read_api_request_payloads, run_arm,
    ...
    fn trajectory_summary_identifies_only_empty_stdin_poll_roundtrips() {
        let steps: Vec<nanocodex_eval::AtifStep> = serde_json::from_value(serde_json::json!([
            {
                "step_id": 1,
                "source": "agent",
                "model_name": "gpt-test",
    ...
    async fn codex_arm_uses_the_native_workspace_verifier_and_event_lifecycle() {
        ...
        let codex = CodexExec::new(&binary, nanocodex::oai::MODEL, "medium")
            .unwrap()
            .api_key("test");
        let task =
        ...
        let report = run_arm(
            ...
            Evaluator::builder(agent)
                ...
                .attempt_agent(move |_attempt, _builder| {
                    Ok::<_, Infallible>(nanocodex_eval::AttemptAgent::codex(configured.clone()))
                }),
            TrajectoryProjection::Codex {
        ...
        let trajectory: nanocodex_eval::AtifTrajectory =
